...
def write_link_map_to_result_file(size_map, link_map_file_path, link_map_result_file_path, analyse_group, target_sdk_name):
    total_size = 0
    target_sdk_total_size = 0;
    a_file_map = {}
    for key in size_map:
        symbol = size_map[key]
        if "size" in symbol:
            ...
            if not analyse_group:
                if symbol["file"].find(target_sdk_name) != -1:
                    target_sdk_total_size += symbol["size"]
                ...
                if symbol.find(target_sdk_name) != -1:
                    target_sdk_total_size += symbol["size"]
                total_size += symbol["size"]
                o_file_name = symbol["file"].split("/")[-1]
    ...
    print ("%s" % "=".ljust(80, '='))
    print ("%s" % (link_map_file_path + "各模块体积汇总").center(87))
    print ("%s" % "=".ljust(80, '='))
    ...
    print ("%s \t\t\t%s \t\t%s\n\n" % ("文件:".ljust(53), "大小(byte):", "占比"))
    output_file.write("%s \t\t\t%s\n\n" % ("文件:".ljust(53), "大小(byte):"))
    tmp_sum = 0
    for item in a_file_sorted_list:
        if item[0].find(target_sdk_name) == -1:
            continue;
        # # 判断文件大小
        # if item[1] / 1024.0 / 1024.0 > 1:
        #     print ("%s%.2fM \t\t%s" % (item[0].ljust(80), item[1] / 1024.0 / 1024.0, ))
        ...
        #     pass
        # else:
        #     print ("%s%.2fK \t\t" % (item[0].ljust(80), item[1] / 1024.0), '{:.4%}'.format(item[1] * 1.0 / target_sdk_total_size))
        #     output_file.write("%s \t\t\t%.2fK\n" % (item[0].ljust(55), item[1] / 1024.0))
        #     tmp_sum += item[1] * 1.0 / target_sdk_total_size
        #     pass
        # pass
        print ("%s%d \t\t" % (item[0].ljust(80), item[1]), '{:.4%}'.format(item[1] * 1.0 / target_sdk_total_size))
        output_file.write("%s \t\t\t%d\n" % (item[0].ljust(55), item[1]))
        tmp_sum += item[1] * 1.0 / target_sdk_total_size
    print ("\n")
    print ("%s%s%.2fK" % (target_sdk_name, "总体积:".ljust(70), target_sdk_total_size / 1024.0))
    print ("%s%.2fM" % ("总体积:".ljust(80), total_size / 1024.0 / 1024.0))
    print ("\n\n %s" % (tmp_sum))
...
def read_link_map_file(base_link_map_file_path, link_map_result_file_path):
    try:
        link_map_file = open(base_link_map_file_path)
        # link_map_file = open(base_link_map_file_path, 'r+', encoding='utf8')
    except IOError:
        ...
        try:
            ...
            write_link_map_to_result_file(size_map, base_link_map_file_path, link_map_result_file_path, False, "audiosdk")
...
def compare_link_map(base_bundle_list, target_bundle_list):
    ...
    for target_bundle_map in target_bundle_list:
        ...
        for base_bundle_map in base_bundle_list:
            ...
            if base_name == target_name:
                ...
                if base_size_value < target_size_value:
                    print ("%s%s%s" % (target_name.ljust(54), str("%.2fK" % base_size_value).ljust(14), str("%.2fK" % target_size_value).ljust(14)))
                    pass
                else:
                    print ("%s%s%s" % (target_name.ljust(54), str("%.2fK" % base_size_value).ljust(14), str("%.2fK" % target_size_value).ljust(14)))
                    pass
        ...
        if has_bundle_in_base == 0:
            print ("%s%s%s%s" % (target_name.ljust(54), str("%.2fK" % base_size_value).ljust(14),
                                            str("%.2fK" % target_size_value).ljust(14), "Y".center(10)))
            pass
        pass
    # for base_bundle_map in base_bundle_list:
    #     base_name = base_bundle_map["name"]
...
def ios_ipa_analyse():
    ...
    # base_map_link_file = "thunder_linkmap_cmp/release_no_flto=full_4.4.txt"
    ...
    if output_file_path:
        ...
        pass
    read_link_map_file(base_map_link_file, base_output_file)
    ...
    if need_compare == 1:
        ...
        if output_file_path:
            ...
            pass
        read_link_map_file(target_map_link_file, target_output_file)
